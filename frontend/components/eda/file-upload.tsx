...
const ACCEPTED_FORMATS = {
  ...
  "application/octet-stream": [".parquet", ".data", ".file"],
};
...
const formatIcons: Record<string, React.ReactNode> = {
  ...
  file: <FileText className="h-4 w-4 text-indigo-500" />,
};
...
export default function FileUpload({ projectId, onJobCreated }: FileUploadProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await api.post<EDAJob>(`/eda/${projectId}/upload`, formData, {
