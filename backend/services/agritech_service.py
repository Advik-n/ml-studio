...
def generate_agritech_report(analysis_results: Dict[str, Any]) -> str:
    """Generate a human-readable text report from agritech analysis results."""
    try:
        L: List[str] = []
        L.append("=" * 70)
        L.append("  AGRITECH IMAGE ANALYSIS REPORT")
        L.append("=" * 70)

        overall = analysis_results.get("overall_health_score", 0)
        status = "HEALTHY" if overall >= 80 else "WARNING" if overall >= 50 else "CRITICAL"
        L.append(f"\nOverall Health Score: {overall}/100  [{status}]")
        ...
        if scores:
            ...
            for cls, sc in sorted(scores.items(), key=lambda x: x[1]):
                ...
                L.append(f"  {cls:30s}  {sc:6.1f}  {bar}")

        for section, key, label in [
            ("disease_indicators", "class", "Disease Indicators"),
            ("pest_indicators", "class", "Pest Damage Indicators"),
            ("environmental_stress", "class", "Environmental Stress"),
        ]:
            items = analysis_results.get(section, [])
            if items:
                L.append(f"\n--- {label} ---")
                for d in items:
                    L.append(f"  [{d['class']}] {d['indicator']} "
                             f"(confidence: {d['confidence']:.0%}) — {d['description']}")

        causes = analysis_results.get("cause_analysis", [])
        if causes:
            L.append("\n--- Cause Analysis ---")
            for c in causes:
                L.append(f"  Type: {c['type']:15s}  Evidence: {c['evidence_count']} indicators")
                L.append(f"    {c['detail']}")
        ...
        if effects:
            L.append("\n--- Effect Analysis ---")
            for e in effects:
                L.append(f"  {e['class']:30s}  Severity: {e['severity']:8s}  "
                         f"Damage: {e['damage_pct']:.1f}%")
        ...
        if impact:
            ...
            L.append(f"  Affected classes:     {impact.get('affected_classes', 0)}"
                     f" / {impact.get('total_classes', 0)}")

        risks = analysis_results.get("future_risks", [])
        if risks:
            L.append("\n--- Future Risk Predictions ---")
            for r in risks:
                L.append(f"  ⚠ {r['risk']}  (likelihood: {r['likelihood']})")
        ...
        if kb:
            ...
            for m in kb:
                L.append(f"  {m['name']} (risk: {m['risk_level']})")
                L.append(f"    Symptoms: {m['symptoms']}  |  Treatment: {m['treatment']}")
        ...
        if sev:
            L.append("\n--- Severity Summary ---")
            L.append(f"  Disease: {sev.get('disease_count', 0)}  "
                     f"Pest: {sev.get('pest_count', 0)}  "
                     f"Environmental: {sev.get('environmental_count', 0)}")
